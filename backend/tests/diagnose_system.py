...
class SystemDiagnostics:
    ...
    def check_vector_store(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if course_count == 0:
                    self.log_warning("Vector store contains no courses - this could explain query failures")
            ...
            try:
                ...
                if test_results.error:
                    self.log_warning(f"Test search returned error: {test_results.error}")
                    store_status['search_test'] = f"error: {test_results.error}"
                else:
                    self.log_success(f"Test search successful, returned {len(test_results.documents)} results")
                    store_status['search_test'] = f"success: {len(test_results.documents)} results"
            ...
            try:
                ...
                for collection in collections:
                    try:
                        ...
                        self.log_info(f"Collection '{collection.name}' has {count} items")
                    except Exception as e:
                        collection_info[collection.name] = f"error: {str(e)}"
                        self.log_warning(f"Error counting collection '{collection.name}': {e}")
    ...
    def check_search_tools(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.log_info(f"Result type: {type(result)}, length: {len(str(result))}")
                tools_status['tool_execution'] = 'success'
                tools_status['result_sample'] = str(result)[:100] + "..." if len(str(result)) > 100 else str(result)
            ...
            try:
                ...
                self.log_success(f"ToolManager initialized with {len(definitions)} tools")
                tools_status['tool_manager'] = 'success'
                tools_status['registered_tools'] = len(definitions)
                
                # Test tool execution through manager
                try:
                    manager_result = tool_manager.execute_tool("search_course_content", query="test")
    ...
    def generate_report(self, all_status: Dict[str, Any]) -> str:
        ...
        if 'vector_store' in all_status and all_status['vector_store'].get('course_count') == 0:
        ...
        if self.issues:
            report.append("• Address all critical issues before expecting queries to work")
